# 🦂 The Burrow — Investigations

![Focus](https://img.shields.io/badge/Focus-Cybersecurity_Investigations-red)
![Scope](https://img.shields.io/badge/Scope-Purple_Team-purple)
![Status](https://img.shields.io/badge/Status-Active-success)

---

## 🕵️‍♂️ Overview

This section contains hands-on cybersecurity investigations conducted within **The Burrow** — a controlled home lab environment designed for **offensive, defensive, and forensic analysis**.

Each investigation reflects real-world techniques across multiple domains, including:

- 🔍 Digital Forensics (DFIR)
- 🌐 Network Analysis
- 🛠️ Vulnerability Research
- ⛓️ Blockchain / OSINT
- 🧪 Lab Notes & Exploratory Testing

---

## 📁 Investigation Categories

### 🔍 DFIR (Digital Forensics & Incident Response)
- [Windows 7 Offline Data Recovery](./dfir/windows7_offline_data_recovery.md)

---

### 🌐 Network Analysis
- [Splunk Forwarder Network Incident](./network-analysis/splunk-forwarder-network-incident/splunk_forwarder_network_segmentation_case_study.md)

- [AnonSurf DNS Failure (Parrot OS Live)](./network-analysis/anonsurf_dns_failure_burrow.md)

---

### 🛠️ Vulnerability Research
- [OpenVAS vs Nessus Scanner Comparison](./vulnerability-research/scanner_comparison_openvas_vs_nessus.md)

---

### ⛓️ Blockchain / OSINT
- [Pi Node Testnet Investigation](./blockchain/pi-node-testnet2/pi_node_security_analysis.md)

---

### 🤖 AI Research
- [HexStrike + Ollama MCP](./ai-research/hexstrike-ai-reliability.md)
- [Mercury-Agent + Ollama Integration Test](./ai-research/mercury-agent-ollama-integration-burrow_a.md)
- [Strix AI Pentest Agent on Krypton1t3](./ai-research/strix_krypton1t3_report_2026-05-08.md)

---

### 👊 Incident Response
- [CVE 2026-31431 Copy.Fail](./incident-response/CVE-2026-31431-copy.fail/README.md)
- [CVE 2026-43284 CVE 2026-43500 Dirty Frag](../incident-response/CVE-2026-43284_CVE-2026-43500_dirty.frag/README.md)

---

### 🧪 Lab Notes
- Ongoing experiments and preliminary testing  
- See: [`lab-notes/`](./lab-notes/)

---

## 🧠 Methodology

Investigations in this lab follow a structured approach:

1. Recon / Discovery  
2. Enumeration / Analysis  
3. Exploitation or Data Collection  
4. Logging & Detection (Splunk, Wazuh)  
5. Documentation & Reporting  

---

## 🦂 Burrow Philosophy

> “Don’t just run tools — understand what they reveal, what they miss, and how to detect them.”

---

## 🚧 In Progress

This section is actively evolving as new investigations are completed and existing ones are expanded with deeper analysis and detection engineering.

---
